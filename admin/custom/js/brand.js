...
$(document).ready(function() {
	...
	$("#submitBrandForm").unbind('submit').bind('submit', function() {
		...
		if (brandStatus === "" || (brandStatus !== "1" && brandStatus !== "2")) {
			$("#brandStatus").after('<p class="text-danger">Select a valid Status</p>');
			$('#brandStatus').closest('.form-group').addClass('has-error');
		} else {
			$("#brandStatus").closest('.form-group').addClass('has-success');	  	
		}
	
		if (brandName && (brandStatus === "1" || brandStatus === "2")) {
			const form = $(this);
			// button loading
			...
			$.ajax({
				...
				success: function(response) {
					...
					if (response.success) {
						...
						// Optionally, scroll to the top of the form or show the form if it's hidden
						$('html, body').animate({
							scrollTop: $("#submitBrandForm").offset().top
						}, 500);
					}
				}
			});
